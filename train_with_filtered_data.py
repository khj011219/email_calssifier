...
def load_and_prepare_filtered_data():
    """필터링된 데이터를 로드하고 전처리합니다."""
    print("필터링된 데이터 로딩 중...")
    
    # 필터링된 데이터 로드
    df = pd.read_csv('dataset/filtered_high_confidence.csv')
    ...
    # 신뢰도 통계
    print(f"\n신뢰도 통계:")
    print(f"평균 신뢰도: {df['confidence'].mean():.4f}")
    print(f"최소 신뢰도: {df['confidence'].min():.4f}")
    print(f"최대 신뢰도: {df['confidence'].max():.4f}")
...
def main():
    print("=== 필터링된 데이터로 BERT 모델 학습 시작 ===")
    
    # 1. 필터링된 데이터 로드 및 전처리
    print("\n1. 필터링된 데이터 로드 및 전처리 중...")
    train_texts, test_texts, train_labels, test_labels, label_to_id, id_to_label = load_and_prepare_filtered_data()
    ...
    training_args = TrainingArguments(
        output_dir="./bert_filtered_model",
        learning_rate=2e-5,
    ...
    results_dict = {
        ...
        'data_source': 'filtered_high_confidence',
    ...
    # 결과 디렉토리 생성
    os.makedirs('./bert_filtered_model', exist_ok=True)
    
    with open('./bert_filtered_model/evaluation_results.json', 'w', encoding='utf-8') as f:
        json.dump(results_dict, f, ensure_ascii=False, indent=2)
    
    # 라벨 매핑 저장
    with open('./bert_filtered_model/label_mapping.json', 'w', encoding='utf-8') as f:
        json.dump({
    ...
    print(f"\n=== 학습 완료 ===")
    print(f"모델이 './bert_filtered_model' 디렉토리에 저장되었습니다.")
