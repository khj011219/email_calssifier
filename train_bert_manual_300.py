...
def load_and_prepare_data():
    ...
    # CSV 파일 로드
    df = pd.read_csv('dataset/filtered_high_confidence.csv')  # 필터링된 높은 신뢰도 데이터셋으로 변경
    ...
    # train_test_split 수행 (50:50 비율로 수정됨)
...
def tokenize_function(examples, tokenizer, max_length=512):
    ...
    return tokenizer(
        ...
        return_tensors=None  # 배치 처리 시 None으로 설정
    )
...
def main():
    ...
    # 1. 데이터 로드 및 전처리
    print("\n1. 데이터 로드 및 전처리 중...")
    train_texts, test_texts, train_labels, test_labels, label_to_id, id_to_label = load_and_prepare_data()
    ...
    training_args = TrainingArguments(
        output_dir="./bert_manual_300_model",
        learning_rate=2e-5,
    ...
    # 결과 디렉토리 생성
    os.makedirs('./bert_manual_300_model', exist_ok=True)
    
    with open('./bert_manual_300_model/evaluation_results.json', 'w', encoding='utf-8') as f:
        json.dump(results_dict, f, ensure_ascii=False, indent=2)
    
    # 라벨 매핑 저장
    with open('./bert_manual_300_model/label_mapping.json', 'w', encoding='utf-8') as f:
        json.dump({
    ...
    print(f"\n=== 학습 완료 ===")
    print(f"모델이 './bert_manual_300_model' 디렉토리에 저장되었습니다.")
